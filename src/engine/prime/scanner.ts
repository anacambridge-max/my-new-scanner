...
function determineState(
  ...
  if (hasReaction && volumeParticipating && emaAligned) return "SETUP";
  if (hasReaction) return "WATCH";
  return "NO_TRADE";
}
...
function buildPipeline(
  ...
  const reactionResult: StageResult = hasReaction
    ? "PASS"
    : reaction.reactionType === "NEUTRAL" ? "WAIT" : "NOT_AVAILABLE";
  const candleResult: StageResult = candle
    ? candle.bodyPercent >= 40 ? "PASS" : "WAIT"
    : "NOT_AVAILABLE";
  ...
  return [
    { name: "LEVEL", result: levelResult, detail: hasLevels ? "YH/YL verified" : "No level data" },
    { name: "REACTION", result: reactionResult, detail: reaction.reactionType },
    { name: "CANDLE", result: candleResult, detail: candle ? `${candle.type} | Body ${candle.bodyPercent.toFixed(0)}%` : "No candle" },
    { name: "VOLUME", result: volumeResult, detail: volume ? `${volume.ratio.toFixed(1)}× avg` : "No volume" },
    { name: "20 EMA", result: emaResult, detail: ema?.ema20 ? `EMA ₹${ema.ema20} | ${ema.position}` : "Insufficient data" },
    { name: "CONFIRMATION", result: "NOT_AVAILABLE", detail: "Rule not verified from Prime Technical manual" },
    {
      ...
      detail: reaction.structuralSL !== null ? `Structural SL ₹${reaction.structuralSL}` : "Awaiting reaction",
    },
...
function buildReason(
  ...
  if (state === "SETUP") parts.push("Confirmation rule NOT VERIFIED per Prime Technical manual — awaiting verified confirmation signal.");
...
export function scanInstrument(input: ScanInput): PrimeScanRow {
  ...
  const volume: VolumeAnalysis | null = candle && candles5m.length > 1
    ? analyseVolume(candle.volume, candles5m.map((c) => c.volume), candle.timestamp)
    : null;
  ...
  // Historical PRIME signal replay is deliberately separate from current status.
  // This lets a stock keep showing the earlier confirmed Pine V7 signal after price moves away from EMA/level.
  const historical = findHistoricalPrimeSignal(candles5m);
  if (historical) {
    const time = new Intl.DateTimeFormat("en-IN", { timeZone: "Asia/Kolkata", hour: "2-digit", minute: "2-digit", day: "2-digit", month: "short" }).format(new Date(historical.signalTimestamp));
    pipeline.push({
      name: "PRIME HISTORY",
      result: "PASS",
    ...
    pipeline.push({ name: "PRIME HISTORY", result: "NOT_AVAILABLE", detail: "No confirmed Pine V7 signal in available 5-min history" });
  }
  ...
  return {
    ...
    location, reaction, candle, volume, ema,
    state, direction, pipeline,
    entry, sl, riskPerShare, qty,
    score, scoreNote, rank: 0, reason, updatedAt, dataFresh,
  };
}
