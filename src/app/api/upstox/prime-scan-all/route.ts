...
export async function GET() {
  ...
  try {
    ...
    const candleTasks = withQuotes.map(instrument => async () => { let raw: unknown[] = []; if (market.status === "CLOSED") { const from = dateDaysAgo(lastSession, 10); raw = await fetchHistoricalCandles(instrument.instrumentKey, token, from, lastSession).catch(() => []); } else { raw = await fetchIntradayCandles(instrument.instrumentKey, token).catch(() => []); } return { symbol: instrument.symbol, intraday: normalizeCandleOrder(raw) }; });
